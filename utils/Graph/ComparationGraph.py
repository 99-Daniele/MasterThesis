...
def hideProcessChosen(choices, sections, subjects, judges, finished, months, changes, sequences, phaseSequences):
    sectionStyle = {'width': 400}
    subjectStyle = {'width': 400}
    judgeStyle = {'width': 400}
    finishedStyle = {'width': 400}
    monthStyle = {'width': 400}
    ...
    if 'finito' in choices:
        ...
        finished = None
    if 'mese' in choices:
        monthStyle = {'width': 200, 'display': 'none'}
        months = None
    ...
    if 'fasi' in choices:
        ...
        phaseSequences = None
    return [sectionStyle, subjectStyle, judgeStyle, finishedStyle, monthStyle, changeStyle, sequenceStyle, phaseSequenceStyle, sections, subjects, judges, finished, months, changes, sequences, phaseSequences]
...
def updateProcessData(df, sections, subjects, judges, finished, months, change, sequences, phaseSequences):
    df_temp = df
    ...
    df_temp = frame.getTypesDataFrame(df_temp, 'mese', months)
...
def displayComparation(df, dateType):
    ...
    app.layout = ds.html.Div([
        ...
        ds.dcc.Dropdown(utilities.months, multi = True, searchable = False, id = 'month-dropdown', placeholder = 'MESE INIZIO', style = {'width': 400}),
        ...
        ds.dcc.Checklist(['sezione', 'materia', 'giudice', 'finito', 'mese', 'cambio', 'sequenza', 'fasi'], value = ['sezione'], id = "choice-checklist", inline = True, style = {'display':'inline'}),
    ...
    @app.callback(
        [ds.Output('comparation-graph', 'figure'),
         ...
         ds.Output('month-dropdown', 'style'),
    ...
    def updateOutput(sections, subjects, judges, finished, months, changes, sequences, phaseSequences, choices, choiceStore, order):
        return comparationUpdate(df, dateType, sections, subjects, judges, finished, months, changes, sequences, phaseSequences, choices, choiceStore, order)
    app.run_server(debug = True)

def comparationUpdate(df, dateType, sections, subjects, judges, finished, months, changes, sequences, phaseSequences, choices, choiceStore, order):
    if len(choices) < 1:
        choices = [choiceStore]
    elif len(choices) == 1:
        choiceStore = choices[0]
    [sectionStyle, subjectStyle, judgeStyle, finishedStyle, monthStyle, changeStyle, sequenceStyle, phaseSequenceStyle, sections, subjects, judges, finished, months, changes, sequences, phaseSequences] = hideProcessChosen(choices, sections, subjects, judges, finished, months, changes, sequences, phaseSequences)
    df_data = df.copy()
    df_data = updateProcessData(df_data, sections, subjects, judges, finished, months, changes, sequences, phaseSequences)
    df_temp = df.copy()
    if ds.ctx.triggered_id != None and 'section-dropdown' in ds.ctx.triggered_id:
        df_temp = updateProcessData(df_temp, None, subjects, judges, finished, months, changes, sequences, phaseSequences)
    elif ds.ctx.triggered_id != None and 'subject-dropdown' in ds.ctx.triggered_id:
        df_temp = updateProcessData(df_temp, sections, None, judges, finished, months, changes, sequences, phaseSequences)
    elif ds.ctx.triggered_id != None and 'judge-dropdown' in ds.ctx.triggered_id:
        df_temp = updateProcessData(df_temp, sections, subjects, None, finished, months, changes, sequences, phaseSequences)
    elif ds.ctx.triggered_id != None and 'sequence-dropdown' in ds.ctx.triggered_id:
        df_temp = updateProcessData(df_temp, sections, subjects, judges, finished, months, changes, None, phaseSequences)
    elif ds.ctx.triggered_id != None and 'phaseSequence-dropdown' in ds.ctx.triggered_id:
        df_temp = updateProcessData(df_temp, sections, subjects, judges, finished, months, changes, sequences, None)
    else:
        df_temp = df_data
...
def displayTypeComparation(df, dateType, type):
    ...
    app = ds.Dash(suppress_callback_exceptions = True)
    app.layout = ds.html.Div([
        ds.html.H2('CONFRONTO DURATA MEDIA FASE 2'),
        ds.dcc.Dropdown(sections, multi = True, searchable = True, id = 'section-dropdown', placeholder = 'SEZIONE', style = {'width': 400}),
        ds.dcc.Dropdown(subjects, multi = True, searchable = True, id = 'subject-dropdown', placeholder = 'MATERIA', style = {'width': 400}),
        ds.dcc.Dropdown(judges, multi = True, searchable = True, id = 'judge-dropdown', placeholder = 'GIUDICE', style = {'width': 400}),
        ds.dcc.Dropdown(utilities.processState, value = [utilities.processState[1]], multi = True, searchable = False, id = 'finished-dropdown', placeholder = 'PROCESSO', style = {'width': 400}),
