...
def durationProcessUpdate(df, datetype, finished, year, sequence, phase, change):
    df_temp = df.copy()
    ...
    [allData, avgData] = frame.getAvgStdDataFrameByDate(df_temp, datetype)
...
def durationCourtHearingsUpdate(df, datetype, finished, year, change):
    df_temp = df.copy()
