...
eventsQuery = "SELECT e.numEvento AS numEvento, e.numProcesso AS numProcesso, en.etichetta AS evento, gn.alias AS giudice, DATE_FORMAT(e.data,'%Y-%m-%d %H:%i:%S') AS dataEvento, sn.etichetta AS stato, sn.fase AS faseStato, p.materia AS materiaProcesso, p.sezione AS sezioneProcesso FROM eventi AS e, processi AS p, eventinome AS en, statinome AS sn, giudicinome AS gn WHERE e.numProcesso = p.numProcesso AND e.codice = en.codice AND e.statofinale = sn.stato AND e.giudice = gn.giudice ORDER BY numProcesso, data, numEvento"
