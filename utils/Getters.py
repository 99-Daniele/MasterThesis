...
eventsQuery = "SELECT e.numEvento AS numEvento, en.etichetta AS tipoEvento, s.stato AS codiceStato, s.fase AS faseStato, e.numProcesso AS numProcesso, e.data AS dataEvento, s.etichetta AS tipoStato, s.abbreviazione AS statoAbbr, p.giudice AS giudiceProcesso, p.materia AS materiaProcesso, p.sezione AS sezioneProcesso FROM eventi AS e, eventinome AS en, statinome AS s, processi AS p WHERE ((e.codice = en.codice) AND (e.statofinale = s.stato) AND (e.numProcesso = p.numProcesso)) ORDER BY e.numProcesso, e.data, e.numEvento"
