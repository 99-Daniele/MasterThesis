...
def displayAllProcesses(cursor):
    findProcesses = "SELECT e.numProcesso, e.data, e.fase FROM eventi AS e WHERE e.numProcesso IN (SELECT numProcesso FROM processifiniti) AND e.numEvento <= ALL (SELECT ev.numEvento FROM eventi as ev WHERE ev.numProcesso = e.numProcesso AND ev.fase = e.fase) ORDER BY numProcesso, fase"
...
def displayProcessesOfYear(cursor, year):
    findProcesses = "SELECT e.numProcesso, e.data, e.fase FROM eventi AS e WHERE e.numProcesso IN (SELECT numProcesso FROM processifiniti WHERE anno = " + str(year) + ") AND e.numEvento <= ALL (SELECT ev.numEvento FROM eventi as ev WHERE ev.numProcesso = e.numProcesso AND ev.fase = e.fase) ORDER BY numProcesso, fase"
    cursor.execute(findProcesses)
    processes = cursor.fetchall()
    for p in processes:
        plt.scatter(p[1], p[0], s = 20, c = colors[p[2] - 1]) 
...
def displayAvgProcessDurationBySection(cursor):
    findDurationEvents = "SELECT sezione, AVG(DATEDIFF(inizioFase5, dataInizio)) FROM processifiniti GROUP BY sezione ORDER BY sezione"
...
def displayAvgProcessDurationByNumberOfEvents(cursor):
    findDurationEvents = "SELECT (SELECT COUNT(*) FROM eventi AS e WHERE p.numProcesso = e.numProcesso GROUP BY e.numProcesso) AS numEventi, AVG(DATEDIFF(dataFine, dataInizio)) FROM processifiniti AS p GROUP BY numEventi ORDER BY numEventi"
    cursor.execute(findDurationEvents)
    processes = cursor.fetchall()  
    counts = []
    avgs = []
    ...
    plt.grid()
    plt.subplots_adjust(0.07, 0.07, 0.97, 0.95) 
    plt.plot(counts, avgs, c = 'black')
    plt.xticks(range(0, 180, 5))
    plt.yticks(range(int(min), int(max),  int((max - min) / 20)))
    plt.xlabel("Numero di eventi")         
    plt.ylabel("[giorni]")
    plt.title("Durata media processi in base al numero di eventi") 
    plt.show()
    ...
    while True:
        ...
        if c == 1:
            ...
            if c2 == 1:
                ...
                displayAvgProcessDurationByNumberOfEvents(cursor)
        elif c == 3:
